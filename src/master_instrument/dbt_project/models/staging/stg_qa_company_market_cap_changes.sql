{{ config(
    ...
    meta = {
        "dagster": {
            "asset_key": ["staging","qa","market","stg_qa_company_market_cap_changes"],
            "group": "market"
        }
    }
) }}
...
SELECT
    ...
    mv."sys_change_operation" AS sys_change_operation,
    mv."sys_change_version"::BIGINT AS sys_change_version,
...
FROM {{ source('raw', 'qa_DS2MktVal_Changes') }} mv
